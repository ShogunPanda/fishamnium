...
function bookmarks_list --description "Lists all bookmarks"
  set query ".+"
...
function bookmarks_autocomplete --description "Autocomplete bookmarks"
  yq -o=tsv -eMP "to_entries | map([.value.bookmark, .value.rootPath | sub(\"\\\$home\", \"\$\$HOME\")])" ~/.fishamnium_bookmarks.json
end

function bookmarks_names --description "Lists all bookmarks names"
  yq -eMP ".[].bookmark"  ~/.fishamnium_bookmarks.json 2>/dev/null
end

function bookmark_show --description "Reads a bookmark"
  # Parse arguments
  set bookmark $argv[1]
...
function bookmark_save --description "Writes a bookmark"
  # Parse arguments
  set bookmark $argv[1]
  set destination $PWD
  set name $argv[2]
...
function bookmark_delete --description "Deletes a bookmark"
  # Parse arguments
  set bookmark $argv[1]
...
function bookmark_move --description "Changes current directory to a saved bookmark"
...
function bookmark_open --description "Edits a bookmark using the current editor"
  if ! set destination (bookmark_show $argv 2>/dev/null)
    echo $destination
    return 1
  end
...
function bookmark_edit --description "Edits a bookmark using the current terminal editor"
  if ! set destination (bookmark_show $argv 2>/dev/null)
    echo $destination
    return 1
  end
