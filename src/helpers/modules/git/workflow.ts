...
export async function workflowStart(args: Arguments, destination: string = null, source: string = null): Promise<void>{
  ...
  // Execute commands
  await workflowDebug(chalk`Creating a new branch {yellow ${branch}} using base branch {yellow ${base}} on remote {yellow ${remote}} ...`);
...
export async function workflowRefresh(args: Arguments, destination: string = null, source: string = null): Promise<void>{
  ...
  // Execute commands
  if(!destination)
    await workflowDebug(chalk`Refreshing current branch {yellow ${branch}} on top of base branch {yellow ${base}} on remote {yellow ${remote}} ...`);
...
export async function workflowFinish(
  ...
  // Debug info
  await workflowDebug(chalk`Merging current branch {yellow ${branch}} to the base branch {yellow ${base}} on remote {yellow ${remote}} ...`);

  if(deleteAfter)
    await workflowDebug(chalk`After merging, the new current branch will be {yellow ${base}} and the current branch {yellow ${branch}} will be deleted.`);
...
export async function workflowFastCommit(args: Arguments): Promise<void>{
  ...
  await workflowDebug(`Committing all changes with message: "${message}" ...`);
...
export async function workflowTagRelease(args: Arguments): Promise<void>{
  ...
  await workflowDebug(chalk`Pushing release tag {yellow ${release}} to the remote {yellow ${remote}} and then deleting the local branch...`);
...
export async function workflowImport(args: Arguments): Promise<void>{
  ...
  // Execute commands
  await workflowDebug(chalk`Deleting eventually existing branch {yellow ${temporary}} ...`);
  ...
  await workflowDebug(chalk`Pushing updated branch {yellow ${destination}} to remote {yellow ${remote}} ...`);
...
export async function workflowPullRequest(args: Arguments): Promise<void>{
  ...
  await workflowDebug(chalk`Creating a pull request from branch {yellow ${branch}} to base branch {yellow ${base}} on remote {yellow ${remote}} ...`);
  await workflowDebug(chalk`After creation, the new current branch will be {yellow ${base}} and the current branch {yellow ${branch}} will be deleted..`);
  ...
  if(url){
    // Open the URL
    await workflowDebug(chalk`Opening URL {yellow ${url}} to finalize the Pull Request creation ...`);
    exec(`${configuration.openPath} "${url}"`);
  }else
